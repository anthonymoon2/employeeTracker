...
function viewAll(tableName: string) {
...
async function addEmployee() {
  ...
  try {
    ...
    const managerID =
      answers.managerID.trim() === "" ? null : answers.managerID;
...
async function addRole() {
  ...
  try {
    ...
    const departmentExists = await checkExistence(
      "department",
    ...
    if (departmentExists) {
      ...
      console.log(
        `Role ${answers.title} with salary ${answers.salary} added with a department ID: ${answers.departmentID} successfully.`
      );
    } else {
